...
from translateMidiseq2 import (encode_lines, keyword_tokens, positions_for, render_lines,
	...
	trim_annotated_prelude, is_elapse_line, is_sustain_line, reconcile_terminators)
...
def check_terminator_handling ():
	'''A terminator the model emits must END the run, and a stray one must never reach the file.

	This is the defect the corpus showed: 485 of 5505 published piano0909 outputs carried a non-final
	`end_of_track`, and 394 of those emitted a SECOND one -- the run rediscovering an ending it had
	already reached, with forced junk in between. `finished` could not stop it because it tests
	`output[-8:]` and the tail between the two terminators runs 31 tokens at p90.

	Three claims:
	  honoured   a terminator generated while the source window holds one ends the run and the tokens
	             after it are dropped, so the output ends ON the terminator.
	  guarded    the same terminator with NO terminator in the source window must not end the run --
	             otherwise a hallucinated one truncates a file that still has source left.
	  stripped   a mid-file terminator that survived to the end is removed, because every corpus file
	             carries exactly one and it is the last line.
	'''
	import torch

	tk = Midiseq2Tokenizer()
	kw = keyword_tokens(tk)
	eot = tk.id_by_token['end_of_track']
	note_on = tk.id_by_token['note_on']
	ok = True

	class ScriptedModel:
		'''Emits a fixed token script per step, then <eos>. `step_scripts` is a list of id lists.'''

		def __init__ (self, vocab, scripts):
			self.vocab, self.scripts, self.emitted, self.step = vocab, scripts, [], 0

		def __call__ (self, ids, mask, pos):
			script = self.scripts[min(self.step, len(self.scripts) - 1)]
			nxt = script[len(self.emitted)] if len(self.emitted) < len(script) else tk.eos_id
			logits = torch.full((1, ids.shape[1], self.vocab), 1.0)
			logits[0, -1, nxt] = 20.0
			self.emitted.append(nxt)
			if nxt == tk.eos_id:
				self.emitted, self.step = [], self.step + 1
			return logits

	vocab = len(tk.tokens)
	# one step that ends the piece and then keeps going -- exactly the observed shape
	script = [[note_on, 0x3c, eot, note_on, 0x3e, note_on, 0x40]]

	# honoured: source ends in end_of_track, so the run must stop AT the terminator
	src = ['note_on #3c $50', 'note_off #3c', 'end_of_track']
	tr = SlidingTranslator(ScriptedModel(vocab, script), tk, max_token=200, prime=False)
	out, stats = tr.translate(src)
	if eot not in out:
		print('  FAIL honoured: the terminator the model emitted is not in the output at all')
		ok = False
	elif out[-1] != eot:
		tail = [tk.tokens[t] for t in out[out.index(eot):]]
		print(f'  FAIL honoured: output continues past the terminator: {tail}')
		ok = False
	if not stats['done']:
		print('  FAIL honoured: the run did not report done, so close_final_measure sees an incomplete bar')
		ok = False
	if stats.get('dropped_after_eot') != 4:
		print(f'  FAIL honoured: dropped_after_eot is {stats.get("dropped_after_eot")}, expected 4')
		ok = False

	# guarded: no terminator in the source, so the model's one must NOT end the run
	src2 = ['note_on #3c $50'] + ['note_on #3e $50'] * 40
	tr2 = SlidingTranslator(ScriptedModel(vocab, script), tk, max_token=200, prime=False)
	out2, stats2 = tr2.translate(src2, max_steps=3)
	if stats2['done']:
		print('  FAIL guarded: a terminator ended the run while the source still had music left')
		ok = False

	# stripped: whatever survived, no mid-file terminator may reach the output
	for name, out_ in (('honoured', out), ('guarded', out2)):
		stray = [i for i, t in enumerate(out_) if t == eot and i != len(out_) - 1]
		if stray:
			print(f'  FAIL {name}: {len(stray)} mid-file end_of_track survived at {stray[:4]}')
			ok = False

	# and the strip itself, isolated: a terminator in the middle goes, the final one stays
	tr3 = SlidingTranslator(ScriptedModel(vocab, script), tk, max_token=200, prime=False)
	kept, n = tr3.strip_stray_terminators([note_on, eot, note_on, eot])
	if n != 1 or kept != [note_on, note_on, eot]:
		print(f'  FAIL strip: removed {n} and kept {[tk.tokens[t] for t in kept]}')
		ok = False
	kept2, n2 = tr3.strip_stray_terminators([note_on, eot])
	if n2 or kept2 != [note_on, eot]:
		print(f'  FAIL strip: a FINAL terminator was removed; that one is the corpus invariant')
		ok = False

	print(f'{"ok  " if ok else "FAIL"} terminator: honoured in-step and tail dropped, '
		f'not honoured without one in the source, no mid-file terminator survives')
	return ok


def check_terminator_agreement ():
	'''The output arm must not claim an ending the source arm does not have.

	The two arms decide their own last line independently, and one direction of disagreement is
	correct: a trimmed run whose source WAS consumed to the last line ends on `@measure` on the
	output arm and on `end_of_track` on the source arm. MEASURED at 224 of 5801 published pairs.

	The inverse is a defect -- `end_of_track` on the output asserts the piece is over while the
	source arm says music was left. MEASURED at 9 of 5801, each one a terminator the run emitted
	before the align stop or a dropped source tail took the rest away.

	The bar count must survive the repair, because the number is the only thing tying the arms
	together: `end_of_track` was closing the last bar and the directive that replaces it closes the
	same bar, so neither is counted and the count does not move.
	'''
	ok = True
	src_open = ['ticks_per_beat 1 e 0', '@measure 1', 'note_on #3c $50', '@measure 2',
		'note_on #3e $50', '@measure 3']
	src_ended = ['ticks_per_beat 1 e 0', '@measure 1', 'note_on #3c $50', '@measure 2',
		'note_on #3e $50', 'end_of_track']
	out_ended = ['ticks_per_beat 1 e 0', '@measure 1', 'note_on #3c $50', '@measure 2',
		'note_on #3e $50', 'end_of_track']
	out_open = ['ticks_per_beat 1 e 0', '@measure 1', 'note_on #3c $50', '@measure 2',
		'note_on #3e $50', '@measure 3']

	def bars (lines):
		n = sum(1 for l in lines if l.startswith('@measure'))
		return n - 1 if lines and lines[-1].startswith('@measure') else n

	# the defect: output ended, source did not -> terminator removed, bar closed by a directive
	fixed, changed = reconcile_terminators(out_ended, src_open)
	if not changed:
		print('  FAIL the unearned terminator was kept; the arms still disagree')
		ok = False
	elif fixed[-1] != '@measure 3':
		print(f'  FAIL the last bar was not closed by a directive: {fixed[-3:]}')
		ok = False
	elif 'end_of_track' in fixed:
		print('  FAIL a terminator survived the repair')
		ok = False
	if bars(fixed) != bars(out_ended):
		print(f'  FAIL bar count moved: {bars(out_ended)} -> {bars(fixed)}; the arms are renumbered')
		ok = False

	# both ended: correct, nothing to do
	for name, out_, src_ in (('both ended', out_ended, src_ended),
			('neither ended', out_open, src_open),
			('source ended, output trimmed', out_open, src_ended)):
		kept, ch = reconcile_terminators(out_, src_)
		if ch or kept != out_:
			print(f'  FAIL {name}: the output arm was rewritten and should not have been')
			ok = False

	# empty input must not raise or invent a line
	for a, b in (([], src_open), (out_ended, []), ([], [])):
		kept, ch = reconcile_terminators(a, b)
		if ch or kept != a:
			print('  FAIL an empty arm was modified')
			ok = False

	print(f'{"ok  " if ok else "FAIL"} terminator agreement: an unearned end_of_track is replaced by '
		f'a closing directive at the same bar count, the other three combinations untouched')
	return ok
...
def main ():
	...
	results = [
		...
		check_eos_mask(),
		check_terminator_handling(),
		check_terminator_agreement(),
