'''Pack the conditioned-MIDI measurewise dataset with the MIDI side in midiseq2 (measure-patches).

Sibling of tools/midi/buildCondMidiMeasurewise.py, but consumes a `midi-seq2/` dir of midiseq2
TEXT (produced by intelli-piano/tools/midiTxtToSeq2.ts from the whole-song MidiText) instead of
the basic `midi-txt/`, and writes the seq2 artifact for starry.midi.data.seq2CondPatchy.

Two-stage flow:
  1. intelli-piano:  midi-txt/*.txt --midiTxtToSeq2.ts-->  midi-seq2/*.txt   (basic -> midiseq2)
  2. here:           dataset.yaml + lyl/ + midi-seq2/  -->  cond-midiseq2.lmmw.pt

Usage:
  python3 tools/midi/buildCondMidiseq2Measurewise.py \
    --dataset ~/data/lilylet/midi-measurewise/test20260629/dataset.yaml \
    --out     ~/data/lilylet/midi-measurewise/test20260629/cond-midiseq2.lmmw.pt
...
from starry.midi.data.seq2CondPachifier import Midiseq2Tokenizer, pack, PATCH_SIZE, LYL_PATCH_SIZE
...
def main ():
	...
	print(f'  patch_size {args.patch_size} | lyl_patch_size {args.lyl_patch_size} | seq2 vocab {seq2_tokenizer.vocab_size}')
	index = pack(samples, lyl_root, midi_seq2_root, lyl_tokenizer, seq2_tokenizer, out_path,
