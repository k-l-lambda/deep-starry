...
optim_dict = dict(map(lambda cls: (cls.name, cls), [
	InvSqrtOptim,
]))


def optim (config, optimizer, init_step):
	optim_type = config['type']
	if optim_type not in optim_dict:
		raise RuntimeError("Optim type %s not found" % optim_type)

	optim_class = optim_dict[optim_type]

	return optim_class(optimizer, init_step=init_step, **config['args'])
