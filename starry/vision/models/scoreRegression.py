...
class ScoreRegressionLoss (nn.Module):
	...
	def forward (self, batch):
		...
		if not self.training:
			...
			metric['y_error'] = (((pred[:, 0] - tar[:, 0]).abs() * mask[:, 0] / self.channel_weights[:, 0]).sum() / mask_sum).item()
			metric['k_error'] = (((pred[:, 1] - tar[:, 1]).abs() * mask[:, 0] / self.channel_weights[:, 1]).sum() / mask_sum).item()
