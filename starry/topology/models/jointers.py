...
class TransformJointer (nn.Module):
	def __init__ (self, d_model=512, d_inner=2048,
			...
			n_type=SemanticElementType.MAX, n_staff=STAFF_MAX):
		super().__init__()

		self.d_model = d_model
		d_word_vec = d_model

		self.source_encoder = Encoder(d_word_vec, n_source_layers, n_head, d_k, d_v, d_model, d_inner, dropout=dropout, scale_emb=scale_emb, n_type=n_type, n_staff=n_staff)
		self.target_encoder = Encoder(d_word_vec, n_target_layers, n_head, d_k, d_v, d_model, d_inner, dropout=dropout, scale_emb=scale_emb, n_type=n_type, n_staff=n_staff)
