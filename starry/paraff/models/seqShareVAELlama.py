...
class LlamaLayerStack (nn.Module):
	def __init__(self, config: LlamaConfig, num_layers: int):
		super().__init__()
	...
	def forward (self, x: torch.Tensor, pos: torch.Tensor, mask: Optional[torch.Tensor] =None):
		if mask is not None:
			# adapt for llama attention_mask's shape: (batch_size, 1, query_sequence_length, key_sequence_length)
			mask = mask[:, None].log()

		for layer in self.layers:
			x = layer(x, attention_mask=mask, position_ids=pos)[0]

		return x


class SeqShareLlamaEncoder (nn.Module):
	def __init__(self, d_model, word_emb, latent_prj_mu, latent_prj_var, dropout, layer_norm, attention, pad_id, finale_id):
		super().__init__()
...
class SeqShareLlamaDecoder (nn.Module):
	def __init__(self, d_model, word_emb, word_prj, latent_emb, dropout, layer_norm, mask_dropout, attention, pad_id):
		super().__init__()
...
class SeqShareVAELlama (nn.Module):
	def __init__ (self, n_vocab, d_latent=256, pad_id=0, finale_id=5,
		...
		dropout=0.1, mask_dropout=0.2, **_):
		super().__init__()
		...
		llama_config = LlamaConfig(hidden_size=d_model, intermediate_size=d_inner, num_attention_heads=n_head)
		self.attention = LlamaLayerStack(llama_config, n_layers)


	def getEncoder (self) -> SeqShareLlamaEncoder:
		return SeqShareLlamaEncoder(d_model=self.d_model, word_emb=self.word_emb,
	...
	def getDecoder (self) -> SeqShareLlamaDecoder:
		return SeqShareLlamaDecoder(d_model=self.d_model, word_emb=self.word_emb, word_prj=self.word_prj, latent_emb=self.latent_emb,
...
class SeqShareVAELlamaLoss (nn.Module):
	def __init__ (self, n_layers, summary_id, kld_weight=0.001, **kw_args):
		...
		self.deducer = SeqShareVAELlama(n_layers=n_layers, **kw_args)
	...
	def forward (self, batch):
		...
		mu, logvar = self.encoder(x, pos)
		z = self.reparameterize(mu, logvar)

		pred = self.decoder(x, pos, z, mask=mask1)
