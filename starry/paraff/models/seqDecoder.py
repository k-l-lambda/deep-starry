...
class SeqDecoderBase (nn.Module):
	...
	def forward (self, seq: torch.Tensor, pos: torch.Tensor, summary: Optional[torch.Tensor] =None, mask: Optional[torch.Tensor] =None):
...
class SeqDecoderBaseLoss (nn.Module):
	...
	def forward (self, batch):
		...
		summary = torch.randn(batch['input_ids'].shape[0], self.deducer.d_model, device=batch['input_ids'].device)

		pred = self.deducer(batch['input_ids'], batch['position'].float(), summary)
