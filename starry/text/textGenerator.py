...
def clipTokenizerWrapInv (tokenizer):
	def run (ids):
		return [tokenizer._convert_id_to_token(id) for id in ids.tolist()]

	return run


def textGenSample (model, tokens2ids, ids2tokens, leading_text='', eos=2, temperature=1, length_limit=1000):
